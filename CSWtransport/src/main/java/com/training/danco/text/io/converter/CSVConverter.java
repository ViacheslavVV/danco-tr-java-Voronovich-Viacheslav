...
public class CSVConverter {
	...
	private static void appendDateAsString(StringBuilder sb, Date date, String delimiter) {
		...
		sb.append(delimiter);
		sb.append(calendar.get(Calendar.MONTH) + MONTH_SHIFT);
		sb.append(delimiter);
	...
	public static Course convertStringToCourse(String string, IFacade facade) {
		...
		Date dateFrom = new GregorianCalendar(Integer.parseInt(params[1]), Integer.parseInt(params[2]),
				Integer.parseInt(params[3]), Integer.parseInt(params[4]), Integer.parseInt(params[5])).getTime();
		Date dateTo = new GregorianCalendar(Integer.parseInt(params[6]), Integer.parseInt(params[7]),
				Integer.parseInt(params[8]), Integer.parseInt(params[9]), Integer.parseInt(params[10])).getTime();
		Course newCourse = new Course(params[0], dateFrom, dateTo, Integer.parseInt(params[11]),
	...
	public static Lection convertStringToLection(String string) {
		String[] params = string.split(SPLIT_DELIMITER);
		Date date = new GregorianCalendar(Integer.parseInt(params[1]), Integer.parseInt(params[2]),
				Integer.parseInt(params[3]), Integer.parseInt(params[4]), Integer.parseInt(params[5])).getTime();
		Lection newLection = new Lection(params[0], date);
