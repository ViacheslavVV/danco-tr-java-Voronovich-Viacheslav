...
public class CurrentCoursesSortedByStudentsCountItem extends MenuItem {
	...
	private static final String GET_CURRENT_COURSES_SORTED_BY_STUDENTS_COUNT = "getCurrentCoursesSortedByStudentsCount";
	private static final Logger LOGGER = LogManager.getLogger(CurrentCoursesSortedByStudentsCountItem.class);

	public CurrentCoursesSortedByStudentsCountItem(Menu menu, IMessageHandler messageHandler) {
		super("Get current courses sorted by number of students.", menu, messageHandler);
	}

	@Override
	public Menu doWork() {
		try {
			Message message = new Message(GET_CURRENT_COURSES_SORTED_BY_STUDENTS_COUNT, null);
