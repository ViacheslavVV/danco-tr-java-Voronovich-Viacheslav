...
public class CoursesAfterDateSortedByStudentsCountItem extends MenuItem {
	...
	private static final String GET_COURSES_AFTER_DATE_SORTED_BY_STUDENTS_COUNT = "getCoursesAfterDateSortedByStudentsCount";
	private static final Logger LOGGER = LogManager.getLogger(CoursesAfterDateSortedByStudentsCountItem.class);
	private static final DateFormat FORMATTER = new SimpleDateFormat(("dd.MM.yyyy"));
	
	public CoursesAfterDateSortedByStudentsCountItem(Menu menu, IMessageHandler messageHandler) {
		super("Get courses after date, sorted by number of students.", menu, messageHandler);
	}
	...
	public Menu doWork() {
		try {
			...
			Message message = new Message(GET_COURSES_AFTER_DATE_SORTED_BY_STUDENTS_COUNT, date);
			...
			if (courses == null || courses.size() == 0) {
				...
				ConsoleEntityDisplayer.displayCourses(courses, "Courses after date " + FORMATTER.format(date) + " sorted by number of students");
			}
		} catch (Exception e) {
